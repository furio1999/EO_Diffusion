...
def main(args):
    ...
    train_dataloader,test_dataloader=create_inria_dataloaders(batch_size=args.batch_size,num_workers=4*ngpu, test=False, size=image_size, length=-1, num_patches=30,
        )
    ...
    for j,(data) in enumerate(test_dataloader):
        ...
        if cond is not None:
            if cond.mean()>=0.9 or cond.mean()<=0.1: continue # original bounds 0.8 and 0.2
        ...
        cond=cond, y=y_test, idx=j)
        samples = F.adjust_brightness(samples, 3) if samples.mean()<0.2 else samples
        if samples.min() < 0: samples = (samples+1.)/2.
    
        if cond is not None and not args.samples_fid:
            if args.cond_type == "sum": 
                image, mask = cond[:,:3], cond[:,3][:,None]
                cond = mask*image 
            gt=image #gt,cond = (image+1.)/2., (cond+1.)/2.
            gt = F.adjust_brightness(gt, 3) if gt.mean()<0.2 else gt
            cond = F.adjust_brightness(cond, 3) if cond.mean()<0.2 else cond
            save_image(gt, gt_path, nrow=int(math.sqrt(args.batch_size)))
            save_image(cond, cond_path, nrow=int(math.sqrt(args.batch_size)))
        ...
        else: save_image(samples,img_path,nrow=int(math.sqrt(args.batch_size)))
