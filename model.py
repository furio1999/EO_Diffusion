...
class MNISTDiffusion(nn.Module):
    def __init__(self,image_size,in_channels,out_channels,cond_channels=0, time_embedding_dim=256,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8], attention_resolutions = tuple([]), num_classes=None):
        super().__init__()
        print("loading model...")
        ...
        self.model = UNetModel(image_size, in_channels=in_channels+cond_channels, model_channels=base_dim, out_channels=out_channels, channel_mult=dim_mults, 
        num_res_blocks=1, attention_resolutions=attention_resolutions, num_classes=num_classes)
        print("Loaded!!")
    ...
    @torch.no_grad()
    def sampling(self,n_samples,clipped_reverse_diffusion=True,device="cuda", cond=None, y=None):
